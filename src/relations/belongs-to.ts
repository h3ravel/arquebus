...
class BelongsTo extends compose(Relation, SupportsDefaultModels) {
  ...
  getRelationExistenceQuery (query: any, parentQuery: any, columns: string[] = ['*']): any {
    ...
    return query
      .select(columns)
      .whereColumn(this.getQualifiedForeignKeyName(), '=', query.qualifyColumn(this.ownerKey as string))
  }


  getRelationExistenceQueryForSelfRelation (query: any, parentQuery: any, columns: string[] = ['*']): any {
  ...
  addConstraints (this: any): void {
    if (this.constructor.constraints) {
      ...
      this.query.where(`${table}.${this.ownerKey}`, '=', this.child[this.foreignKey as string])
    }
  }
