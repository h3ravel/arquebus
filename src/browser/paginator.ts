...
class Paginator<T extends Model, K extends IPaginatorParams = IPaginatorParams> implements IPaginator<T, K> {
    ...
    constructor(items: T[], total: number, perPage: number, currentPage: number = 1, options: TGeneric = {}) {
    ...
    map (callback: (value: T, index: number) => T) {
        return this._items.map(callback) as Collection<T> as any
    }
    currentPage () {
    ...
    toData () {
        if ((this.constructor as any).formatter && typeof (this.constructor as any).formatter === 'function') {
