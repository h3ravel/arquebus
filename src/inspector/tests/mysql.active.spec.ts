...
describe.runIf(mysqlUp)('mysql', () => {
  ...
  beforeAll(async () => {
    ...
    return database.schema
      ...
      .createTable('users', function (table) {
        ...
        table
          .string('email', 100)
          .unique({ indexName: 'team_id_email_unique' })
          .notNullable()
        table.string('password', 60).notNullable()
        table.string('status', 60).defaultTo('active').notNullable()
        table
          .foreign('team_id', 'fk_team_id')
          .references('id')
          .inTable('teams')
          .onDelete('CASCADE')
          .onUpdate('CASCADE')
  ...
  describe('.columnInfo', () => {
    ...
    it('returns information for a specific column in a specific table', async () => {
      expect(await inspector.columnInfo('teams', 'uuid')).toMatchObject({
        name: 'uuid',
        table: 'teams',
        data_type: 'char',
        collation: 'latin1_swedish_ci',
