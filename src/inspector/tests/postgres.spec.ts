...
describe('postgres-no-search-path', () => {
  ...
  const connection = arquebus.fire(config.postgres.client)
  ...
  describe('.columns', () => {
    it('returns information for all tables', async () => {
      database.transaction(async (trx) => {
        expect(
          await schemaInspector.inspect(trx).columns(),
  ...
  describe('.transaction', () => {
    it('works with transactions transaction', async () => {
      database.transaction(async (trx) => {
        expect(await schemaInspector.inspect(trx).primary('teams')).to.equal(
          'id',
        )
      })
    })
...
describe('postgres-with-search-path', () => {
  ...
  describe('.transaction', () => {
    it('works with transactions transaction', async () => {
      database.transaction(async (trx) => {
        expect(await schemaInspector.inspect(trx).primary('test')).to.equal(
          'id',
        )
      })
    })
