    ...
    it('returns information for all tables', async () => {
      database.transaction(async (trx) => {
        expect(await schemaInspector(trx).columns()).to.have.deep.members([
          ...
          { column: 'team_id', table: 'users' },
          { column: 'email', table: 'users' },
          { column: 'password', table: 'users' },
          { column: 'status', table: 'users' },
          { column: 'primaryKey', table: 'camelCase' },
          { column: 'request_path', table: 'page_visits' },
          { column: 'user_agent', table: 'page_visits' },
          { column: 'created_at', table: 'page_visits' },
          { column: 'domain', table: 'detailed_page_visits' },
          { column: 'request_path', table: 'detailed_page_visits' },
          { column: 'user_agent', table: 'detailed_page_visits' },
        ])
      })
